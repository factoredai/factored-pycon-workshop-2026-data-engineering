...
**Knowing which mode you got.** The CLI's `__main__` block calls `load_dotenv()` (from `python-dotenv`,
now in `requirements.txt`) before extracting, so a key in `.env` enables live mode without an `export`
(dotenv runs `override=False`, so a real shell export still wins). It then prints
`describe_mode()` (in `llm_client.py`): `MODE: LIVE (<provider>)` or `MODE: CANNED (no API key found)`.
`load_dotenv()` lives only in the CLI entrypoint, never at import time, so `pytest` stays canned
regardless of your `.env`.
