...
Exercise 3 teaches: hash the input so re-runs skip expensive LLM calls.
Upsert by business key so running twice produces zero duplicates. The
INSERT ... ON CONFLICT DO UPDATE pattern is the direct analog of
...
def hash_content(content: str, prompt_version: str = PROMPT_VERSION) -> str:
    """SHA-256 of (prompt_version, content). Cache key for LLM extractions.

    Combine both inputs into a single string (e.g., f"{prompt_version}:{content}")
    and return the hex digest. Changing PROMPT_VERSION invalidates all cached
    responses, which is the correct behavior when the extraction prompt changes.

    Workshop blank A (core).
    """
    combined = f"{prompt_version}:{content}"
...
def upsert_result(
    ...
    """Insert or update result by business key. Run twice, zero duplicates.

    Same pattern as MERGE in Snowflake/BigQuery. Look at init_db() for the
    table schema. Returns True if a new row was inserted, False if updated.
    ...
    cursor.execute(
        """
        INSERT INTO results (reference_id, buyer, process_date, verdict, review_reasons, findings)
        VALUES (?, ?, ?, ?, ?, ?)
        ON CONFLICT(reference_id, buyer, process_date)
        DO UPDATE SET
        ...
        (
            ...
            json.dumps(verdict.review_reasons),
            json.dumps([f.model_dump() for f in verdict.findings]),
        ),
    )
...
def process_with_cache(
    ...
    """Full idempotent pipeline. Provided to attendees (wires everything together)."""
    init_db(db_path)
