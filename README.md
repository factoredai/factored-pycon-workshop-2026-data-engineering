...
DAG. The `python -m workshop.pipeline.extract` CLI loads `.env` for you via `python-dotenv`
(installed by `requirements.txt`), so a key in `.env` is enough: no `export` needed. Docker
Compose forwards the key into the containers the same way. A real shell `export` still wins over
`.env` (dotenv runs with `override=False`). Precedence when several keys are set: Anthropic, then
OpenRouter, then OpenAI.
...
- **Exercise 1 (extraction), no Docker:** once your tests pass, run the extractor on one PDF and
  ...
  ```
  The CLI prints a `MODE: LIVE (<provider>)` or `MODE: CANNED (no API key found)` banner up front,
  so you can confirm which path a run used without reading the logs.
